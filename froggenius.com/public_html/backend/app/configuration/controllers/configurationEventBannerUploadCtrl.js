...
angular.module('newApp')
    .controller('configurationEventBannerUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_configuration_event_banner")
        });
        ...
        img_uploader.onCompleteItem = function(fileItem, response, status, headers) {
            ...
            $scope.configuration_data.event_banner = response.file_name;
