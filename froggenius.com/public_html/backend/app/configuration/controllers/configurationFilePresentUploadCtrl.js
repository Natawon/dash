...
angular.module('newApp')
    .controller('configurationFilePresentUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_configuration_file_present")
        });
        ...
        img_uploader.onCompleteItem = function(fileItem, response, status, headers) {
            ...
            $scope.configuration_data.file_present = response.file_name;
