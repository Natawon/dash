...
angular.module('newApp')
    .controller('configurationLogoUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_configuration_logo")
        });
        ...
        img_uploader.onCompleteItem = function(fileItem, response, status, headers) {
            ...
            $scope.configuration_data.logo = response.file_name;
