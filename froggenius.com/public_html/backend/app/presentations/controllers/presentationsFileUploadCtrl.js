...
angular.module('newApp')
    .controller('presentationsFileUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_presentations_file")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            $scope.presentations_data.file = response.file_name;
        };
