...
function getImage($dir, $images, $code, $cls) {
    if ($images) {
        // $path = $dir.$images;
        $path = '<img src="'.$dir.$images.'" class="'.$cls.'" alt="">';
    } else {
        $path = '<div class="bg-gray-3 h-100 w-100 d-flex flex-column justify-content-center align-items-center rounded-top">
                    <i class="fas fa-chalkboard text-white fa-2x"></i>
                    <div class="text-white mt-2">'.$code.'</div>
                </div>';
...
function desc_sort_json_state($array1, $array2){
    $key = 'state';

    if ($array1['topics'][$key] == $array2['topics'][$key]) {
        return 0;
    }
    return ($array1['topics'][$key] < $array2['topics'][$key]) ? 1 : -1;
}

function desc_sort_json_streaming_status($array1, $array2){
...
function desc_agenda_streaming_status($array1, $array2){
    $key = 'streaming_status';
