...
/*
	---------------------------------------------------------------
		...
		PHP Scripted FFmpeg Video Converter 2.2.5
		By Kenny Svalgaard
		http://sye.dk/
...
*/
	...
	function echoTime($str, $onlyLog=false, $lineBreakFirst=false)
	{
		$str = ($lineBreakFirst?"\r\n\r\n":'').date('Y-m-d H:i:s', time()+UTC_OFFSET_SEC).' '.$str."\r\n";
		if (!$onlyLog)
		{
			echo $str;
		}
	...
	function probeVideo($from)
	{
		...
		foreach ($probeArray['streams'] as $pro)
		{
			if (($pro['codec_type'] === 'video') and !$videoStream)
			{
				...
				if ($avgFrameRate !== false)
				{
					if (is_nan($avgFrameRate) or (!is_float($avgFrameRate) and !is_int($avgFrameRate))) // sometimes a video have "0/0" as avg_frame_rate
					{
						$avgFrameRate = false;
					}
				}
				$duration = (isset($pro['duration'])?(int)$pro['duration']:false);
				if (isset($pro['tags']['rotate']))
				{
					$rotate=(int)$pro['tags']['rotate'];
		...
		if ($convert)
		{
					...
					if ($char == "\r")
					{
						...
						if (($charFrom !== false) and ($charTo !== false))
						{
							...
							if ($percentFactor)
							{
								...
								{
									$progressBar .= ((($i*$progressBarStep)+($progressBarStep/2)<$percentColplete)?'■':' ');
								}	
			...
			if ($returnVar === 0)
			{
				if ($percentFactor)
				{
					...
					echo niceTime($timeTotal).'    '.niceTime($timeElapsed).'    '.niceTime(0).'    '.'100%'.' ['.$progressBar.'] '."\r";
				}
				echo "\n\n";
				$fileSizeFrom = filesize($from);
				$fileSizeTo = filesize($to);
	...
	function generateScreenshot($path, $videoFile, $videoDuration)
	{
		...
		else
		{
			...
			$cmd='.\system\ffmpeg.exe '.$param.' 2>&1';
			echoTime($cmd, true);
			exec($cmd);
		}
	}
	...
	echoTime('PHP Scripted FFmpeg Video Converter started.', false, !CLEAR_LOG_FILE);
	...
	if (isset($argv[1]))
	{
		...
		{
			abort('Settings file does not exist: '.$argv[1]);
		}
	}
	else
	{
