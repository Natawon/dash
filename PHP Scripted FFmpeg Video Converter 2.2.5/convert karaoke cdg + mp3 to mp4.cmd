...
[x] move	: Select to move source video file to DIR_DONE.
[ ] delete	: Select to delete source file.
...
[x] move	: Move source video file to DIR_FAILED.
...
< DIR_OUT_ACTION >
[x] move		: Move source file to DIR_EXISTED.
[ ] rename		: Use a new unique name for destination file.
[ ] overwrite	: Overwrite destination file.
[ ] delete		: Skip conversion/move and delete source file.
...
< DIR_DONE_ACTION >
[x] rename		: Use a new unique name for destination file.
[ ] overwrite	: Overwrite destination file.
[ ] delete		: Skip move and delete source file.
...
[ ] {width} * {height} * {fps} * 0.15	: High bit rate. 	For high motion video.				(a video at 1920 x 1080 with 25 fps will get a bit rate at 7776 kbps)
[x] {width} * {height} * 25 * 0.09		: For cdg karaoke songs. Setting have no fps, as cdg files normally have a very high fps, so setting to 25 fixed.
[ ]										: Add custom setting before the colon, and move the x to this line to use the setting.
...
< VIDEO_EXTENSIONS >
[ ] asf, avi, mpeg, mpg, wmv, mkv, mp4, m4v, mp4v, flv, mov, ts, swf, f4v, rm, 3g2, 3gp2, gp, 3gpp		: Commonly used extensions for videos.
[x] cdg							: cdg files for karaoke songs
[ ]								: Add custom setting before the colon, and move the x to this line to use the setting.
...
[x] true	: Use the EXCLUDE_EXTENSIONS.
[ ] false	: Default. Do not use the EXCLUDE_EXTENSIONS.
...
< FFMPEG_PARAMETERS >
[ ] -i "{from}" -b:v {videobit} -vcodec libx264 -preset slow -vf scale={width}:{height} -threads 0 -b:a {audiobit} "{to}"	: Convert to mp4, auto select audio codec
...
[x] -i "{from}" -i "{from_no_ext}.mp3" -b:v {videobit} -vcodec libx264 -vf scale={width}:{height} -b:a {audiobit} -pix_fmt yuv420p "{to}"			: Merging cdg karaoke lyrics and mp3 to mp4
[ ] 						: Add custom setting before the colon, and move the x to this line to use the setting.
...
[ ] true	: Generate screenshots.
[x] false	: Do not generate screenshots.
